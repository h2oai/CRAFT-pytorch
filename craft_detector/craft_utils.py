...
# TODO - Remove linkmap. never used in code
def get_poly_core(boxes, labels, mapper, linkmap):
    ...
    for k, box in enumerate(boxes):
        ...
        # TODO Fix too broad exception handling
        try:
        ...
        for i in range(0, len(cp)):
            ...
            if (seg_num + 1) * seg_w <= x and seg_num <= tot_seg:
                ...
                if num_sec == 0:
                    break
                # TODO Fix the Unexpected Types error
        ...
        # TODO Fix cannot find reference error in the next two lines
        ...
        # make final polygon
        poly = [wrap_coord(minv, (spp[0], spp[1]))]
        for p in new_pp:
            poly.append(wrap_coord(minv, (p[0], p[1])))
        poly.append(wrap_coord(minv, (epp[0], epp[1])))
        poly.append(wrap_coord(minv, (epp[2], epp[3])))
        for p in reversed(new_pp):
            poly.append(wrap_coord(minv, (p[2], p[3])))
        poly.append(wrap_coord(minv, (spp[2], spp[3])))
